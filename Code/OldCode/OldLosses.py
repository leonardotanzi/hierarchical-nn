...
def hierarchical_cc_tree(predicted, actual, medium_labels, coarse_labels, tree, n_medium_class, n_coarse_class,
    ...
    if hierarchical_loss:
        ...
        matrixes = return_matrixes(tree, plot=True)

        medium_labels = torch.tensor(medium_labels).type(torch.int64).to(device)
        predicted_medium = torch.matmul(predicted, torch.tensor(matrixes[1]).type(torch.float32).to(device))
        ...
        predicted_coarse = torch.matmul(predicted, torch.tensor(matrixes[0]).type(torch.float32).to(device))
