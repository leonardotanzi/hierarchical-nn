...
def show_tables_acc_newp():
    ...
    # CIFAR
    nsamples = [3.5, 7, 14, 28, 56, 102, 204]
    # 1, MC
    resnet_fine_MC = [1 - i for i in [0.20, 0.28, 0.37, 0.39, 0.45, 0.52, 0.56]]
    resnet_hier_MC = [1 - i for i in [0.20, 0.29, 0.38, 0.47, 0.48, 0.58, 0.69]]
    inception_fine_MC = [1 - i for i in [0.16, 0.22, 0.27, 0.31, 0.39, 0.60, 0.65]]
    inception_hier_MC = [1 - i for i in [0.18, 0.25, 0.32, 0.47, 0.50, 0.64, 0.71]]
    vit_fine_MC = [1 - i for i in [0.04, 0.06, 0.06, 0.12, 0.15, 0.88, 0.88]]
    vit_hier_MC = [1 - i for i in [0.03, 0.07, 0.11, 0.13, 0.20, 0.88, 0.90]]
    # 2, HMC
    resnet_fine_HMC = [0.67, 0.62, 0.56, 0.56, 0.51, 0.47, 0.45]
    resnet_hier_HMC = [0.65, 0.57, 0.51, 0.47, 0.47, 0.42, 0.37]
    inception_fine_HMC = [0.68, 0.65, 0.63, 0.60, 0.56, 0.43, 0.41]
    inception_hier_HMC = [0.64, 0.61, 0.55, 0.47, 0.46, 0.40, 0.36]
    vit_fine_HMC = [0.86, 0.85, 0.84, 0.78, 0.77, 0.30, 0.30]
    vit_hier_HMC = [0.84, 0.80, 0.76, 0.75, 0.67, 0.30, 0.29]

    # imagenet
    # nsamples = [9.5, 19, 37, 74, 148, 296]
    # # 1, MC
    # resnet_fine_MC = [1 - i for i in [0.04, 0.06, 0.11, 0.14, 0.19, 0.23]]
    # resnet_hier_MC = [1 - i for i in [0.05, 0.08, 0.14, 0.23, 0.36, 0.33]]
    # inception_fine_MC = [1 - i for i in [0.03, 0.05, 0.13, 0.20, 0.30, 0.37]]
    # inception_hier_MC = [1 - i for i in [0.20, 0.31, 0.33, 0.36, 0.47, 0.55]]
    # vit_fine_MC = [1 - i for i in [0.02, 0.03, 0.05, 0.09, 0.11, 0.62]]
    # vit_hier_MC = [1 - i for i in [0.06, 0.08, 0.14, 0.31, 0.30, 0.76]]
    # # 2, HMC
    # resnet_fine_HMC = [0.10, 0.11, 0.09, 0.10, 0.09, 0.09]
    # resnet_hier_HMC = [0.08, 0.08, 0.06, 0.06, 0.04, 0.04]
    # inception_fine_HMC = [0.11, 0.08, 0.07, 0.06, 0.06, 0.05]
    # inception_hier_HMC = [0.06, 0.05, 0.04, 0.04, 0.04, 0.03]
    # vit_fine_HMC = [0.28, 0.17, 0.14, 0.12, 0.12, 0.03]
    # vit_hier_HMC = [0.08, 0.07, 0.07, 0.05, 0.05, 0.02]
